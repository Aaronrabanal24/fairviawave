  ...
  if (error) {
    return (
      <Card className="w-full border-red-200">
        <CardHeader>
          <CardTitle className="text-red-600">⚠️ Score Data Error</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right text-sm text-gray-600">
            <div className="font-semibold text-lg text-gray-900">{avgScore}</div>
            <div className="text-xs">7-day avg</div>
          </div>
        </div>
        <div className="text-sm text-gray-600">
          📊 {totalSignals} signals this week
        </div>
      </CardHeader>
      <CardContent>
            ...
            return (
              <div key={day.date} className="flex flex-col items-center flex-1 group">
                <div 
                  ...
                  style={{ height: `${height}%` }}
                  title={`${new Date(day.date).toLocaleDateString()}: ${day.score} score, ${day.signals} signals`}
                />
        ...
        <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            <div className="font-semibold text-gray-900">{Math.max(...data.map(d => d.score))}</div>
            <div className="text-gray-500 text-xs">Peak</div>
          </div>
          <div className="text-center">
            <div className="font-semibold text-gray-900">{Math.min(...data.map(d => d.score))}</div>
            <div className="text-gray-500 text-xs">Low</div>
          </div>
          <div className="text-center">
            <div className="font-semibold text-gray-900">{data[data.length - 1]?.score || 0}</div>
